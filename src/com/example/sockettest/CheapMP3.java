...
public class CheapMP3 extends CheapSoundFile {
    ...
    public void WriteFile(File outputFile, int startFrame, int numFrames) throws java.io.IOException {
    	Log.w("CHEAPMP3","WRITING FILE");
        outputFile.createNewFile();
        FileInputStream in = new FileInputStream(mInputFile);
        FileOutputStream out = new FileOutputStream(outputFile);
        int maxFrameLen = 0;
        
        if((startFrame + numFrames - 1) >= mNumFrames) {
        	Log.w("CHEAPMP3","ADJUST NUMFRAMES FROM: " + numFrames +" TO: " + (mNumFrames - startFrame));
        	numFrames = mNumFrames - startFrame;
        }
   ...
   /*
    public byte[] WriteBuffer(int startFrame, int numFrames) throws java.io.IOException {
        FileInputStream in = new FileInputStream(mInputFile);
        int maxFrameLen = 0;
        for (int i = 0; i < numFrames; i++) {
            if (mFrameLens[startFrame + i] > maxFrameLen)
    ...
    */
    public byte[] writeBuffer(int startFrame, int numFrames) throws java.io.IOException {
        ...
        for (int i = 0; i < numFrames; i++) {
            ...
            Log.w("MP3READER","FRAME " + (startFrame + i) + "\t START: " + pos + " END: " + (len + pos));
